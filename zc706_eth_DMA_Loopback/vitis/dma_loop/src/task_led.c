    ...
    for(;;) {
        if (g_CurrentMode == MODE_WAVE_OSCILLO && g_pc_connected) {
            ...
            if (status == XST_SUCCESS) {
                ...
                if (g_udp_sock >= 0) {
                    int sent = sendto(g_udp_sock, (const void*)RxBufferPtr, TEST_LEN, 0,
                               (struct sockaddr *)&g_pc_addr, sizeof(g_pc_addr));
                    if(sent < 0) xil_printf("UDP Send Error!\r\n"); // [新增调试]
                }
            } else {
                xil_printf("DMA Kickoff Failed! Status: %d\r\n", status);
            }
        }
