...
void Task_DMA_Wave(void *pvParameters) {
    ...
    char temp_buf[16];
    const int CAPTURE_LEN = 1024*4;
    const int SEND_LEN = 256;

    // 定义一个计数器，防止刷屏
    int print_cnt = 0;

    vTaskDelay(pdMS_TO_TICKS(5000));
    xil_printf(">> Task_DMA_Wave Started!\r\n"); // 打印 1

    for(;;) {
        if (g_CurrentMode == MODE_WAVE_OSCILLO && g_pc_connected) {

            // 只有前几次打印，防止刷屏
            if(print_cnt < 5) xil_printf("[%d] Starting DMA Transfer...\r\n", print_cnt);

            Xil_DCacheInvalidateRange((UINTPTR)RxBufferPtr, CAPTURE_LEN);

            int Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR)RxBufferPtr,
                                            CAPTURE_LEN, XAXIDMA_DEVICE_TO_DMA);
            ...
            if (Status == XST_SUCCESS) {
                ...
                while (XAxiDma_Busy(&AxiDma, XAXIDMA_DEVICE_TO_DMA)) {
                    vTaskDelay(1);
                    timeout++;
                    // 如果超过 1秒 (1000ms) 还在忙，说明卡死了！
                    if(timeout > 1000) {
                        if(print_cnt < 5) xil_printf("!! DMA TIMEOUT (Stuck waiting for TLAST) !!\r\n");
                        break;
                    }
                }

                if(timeout <= 1000) {
                    if(print_cnt < 5) xil_printf("[%d] DMA Done. First Byte: 0x%02X\r\n", print_cnt, RxBufferPtr[0]);

                    // 只有成功了才发 UDP
                    Xil_DCacheInvalidateRange((UINTPTR)RxBufferPtr, CAPTURE_LEN);

                    // (为了简化调试，这里先暂时用简单的发送逻辑)
                    strcpy(udp_big_buf, "[WAVE] ");
                    for(int i=0; i<SEND_LEN; i++) {
                         // 简单的触发逻辑，避免全0导致触发失败
                        int8_t val = (int8_t)RxBufferPtr[i*4];
